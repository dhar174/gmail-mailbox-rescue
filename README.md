...
Intel native builds use `macos-x86_64` in the name; Intel is not yet qualified. Generic ZIPs omit `client_secret.json`. Keep approved configured ZIPs private. Tokens, mail, and export/checkpoint data are never release inputs.

The macOS CI job runs the full suite and Ruff on `macos-15` arm64, initializes the source Qt UI offscreen, builds the `.app`, and uploads the generic versioned ZIP as the `Mailbox-Rescue-macos-arm64` workflow artifact. It also extracts that ZIP into a path with spaces, opens the actual app through macOS Launch Services with the native UI, captures its onscreen window, and checks normal quit. The disconnected window screenshot is uploaded separately for inspection. Download and extract the ZIP artifact wrapper to obtain the distribution ZIP. A maintainer must provide the approved sidecar separately or create a private configured build before coworker sign-in.
