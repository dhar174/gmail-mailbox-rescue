...
The macOS CI job runs the full suite and Ruff on `macos-15` arm64, initializes the source Qt UI offscreen, builds the `.app`, and uploads only the generic versioned ZIP as the `Mailbox-Rescue-macos-arm64` workflow artifact. Download and extract that artifact wrapper to obtain the distribution ZIP. A maintainer must provide the approved sidecar separately or create a private configured build before coworker sign-in.
